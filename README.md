...
For the full data-flow and component diagram (config → PMU → learning → detection → alerts, plus ML hybrid), see **[PIPELINE.txt](PIPELINE.txt)**.

---

## Pipeline diagram — `PIPELINE.txt`

The repository includes a **full system pipeline document** in ASCII form:

| File | Description |
|------|-------------|
| **[PIPELINE.txt](PIPELINE.txt)** | End-to-end pipeline: config → init → PMU/telemetry → learning → detection → alerts, plus ML hybrid flow and module dependency map. |

**Contents of PIPELINE.txt:**

- **Working directory layout** — Project tree (`src/`, `ml/`, `tests/`, `bin/`, config files).
- **Configuration** — Inputs (guardian.conf, CLI, defaults), priority order, `guardian_config_t`.
- **Subsystem initialization** — Logger, ring buffer, telemetry, anomaly engine, correlation engine.
- **Background thread** — PMU counters, sampling loop, delta/derived metrics, lock-free ring buffer.
- **Phase 1: Learning** — `anomaly_learn` / baseline, `anomaly_finalize_baseline`, privilege drop.
- **Phase 2: Detection** — Z-score, thresholds, burst/oscillation, composite score, alert levels, correlation, JSON output.
- **Shutdown** — Signal handling, telemetry stop, cleanup order.
- **Detection summary** — C (z-score) vs Python ML (Isolation Forest + One-Class SVM).
- **ML hybrid** — IPC wire format, feature engineering, ensemble inference, periodic retrain.
- **Module dependency map** — `main.c` and all C modules; link to `guardian_ml.py`.
- **How to run** — C-only vs hybrid (ML socket).

Use it as the single reference for data flow and component roles. No build step; open as plain text.
